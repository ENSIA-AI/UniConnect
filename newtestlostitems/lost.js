...
function loadItemsFromDatabase() {
    fetch('lostandfound.php?status=lost')
        .then(response => response.json())
        .then(data => {
            if (Array.isArray(data) && data.length > 0) {
                sampleLostItems.length = 0;
                sampleLostItems.push(...data);
                displayLostItems(sampleLostItems);
            }
        })
        .catch(err => console.log('Could not load lost items from database:', err));
    
    fetch('lostandfound.php?status=found')
        .then(response => response.json())
        .then(data => {
            if (Array.isArray(data) && data.length > 0) {
                sampleFoundItems.length = 0;
                sampleFoundItems.push(...data);
                displayFoundItems(sampleFoundItems);
            }
        })
        .catch(err => console.log('Could not load found items from database:', err));
}
        ...
        try {
            ...
            if (data.error) {
                ...
                const newItem = { 
                    ...
                    date_lost_found: date, contact_email: contact, image_url: null, status: 'lost',
        ...
        try {
            ...
            if (data.error) {
                ...
                const newItem = { 
                    id: data.item_id, 
                    title, category, description, location, 
                    date_lost_found: date, contact_email: contact, storage_location: storage, 
                    image_url: null, status: 'found',
                    created_at: new Date().toISOString(), updated_at: new Date().toISOString()
                };
...
function handleEditFormSubmit(e) {
    ...
    .then(data => {
        ...
        if (data.status === 'success') {
            ...
            const lostIndex = sampleLostItems.findIndex(item => item.id == itemId);
            if (lostIndex !== -1) {
                sampleLostItems[lostIndex] = {
                    ...sampleLostItems[lostIndex],
                    ...itemData,
                ...
                const foundIndex = sampleFoundItems.findIndex(item => item.id == itemId);
                if (foundIndex !== -1) {
                    sampleFoundItems[foundIndex] = {
                        ...sampleFoundItems[foundIndex],
                        ...itemData,
                        updated_at: new Date().toISOString()
                    };
...
function deleteItem(itemId) {
    ...
    fetch('lostandfound.php?id=' + itemId, {
        method: 'DELETE',
        headers: {
            'Authorization': 'Bearer ' + (localStorage.getItem('auth_token') || '')
        }
    ...
    .then(data => {
        ...
        if (data.status === 'success' || data.message) {
            ...
            const lostIndex = sampleLostItems.findIndex(item => item.id == itemId);
            if (lostIndex !== -1) {
                sampleLostItems.splice(lostIndex, 1);
                displayLostItems(sampleLostItems);
            }

            const foundIndex = sampleFoundItems.findIndex(item => item.id == itemId);
            if (foundIndex !== -1) {
                sampleFoundItems.splice(foundIndex, 1);
                displayFoundItems(sampleFoundItems);
            }
